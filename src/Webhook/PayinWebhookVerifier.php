 ...
 * @description : 代收异步通知签名校验器，负责按网关规则拼接 t、tradeNo、orderNo、currency、amount、status、code、message 并计算 SHA-256 hex。本类只做本地签名摘要计算和常量时间比较，不接收 HTTP、不落库、不修改资金、不推进交易状态。
 * @status : modify
 */
final class PayinWebhookVerifier
{
     ...
     * 构建代收异步通知签名原文。
     *
     * 签名原文为 t + tradeNo + orderNo + currency + amount + status + code + message。
     *
     * @param string $timestamp Header t。
     * @param array $request 代收回调参数。
     * @return string 签名原文。
